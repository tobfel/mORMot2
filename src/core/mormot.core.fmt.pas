...
type
  ...
  // - by default, a '@name' field is written as a XML attribute of its enclosing
  // element, and a '#text' field as the element text content, i.e. the very
  // conventions generated by XmlToVariant() - so that JsonToXml(XmlToJson(x))
  // returns the original XML content
  // - jxoNoAttribute and jxoNoText disable this mapping, and write those fields
  // as regular elements - i.e. as '<@name>' and '<#text>', which are not
  // well-formed XML, as it was done before those options were introduced
  TJsonToXmlOption = (
    jxoNoAttribute,
    jxoNoText);
...
// e.g. '<content>'
// - '@name' and '#text' fields are written as attributes and text content, as
// generated by XmlToVariant() - see TJsonToXmlOptions to disable this mapping
...
// - unless jxoNoAttribute/jxoNoText are set, follows the XmlToVariant()
// conventions: a '@name' field with a scalar value is written as a XML
// attribute of its enclosing element, and a '#text' field as the element text
// content - note that '@name' fields should appear before any content field of
// their object, which is how XmlToVariant() does generate them
...
function AddJsonToXmlNode(W: TTextWriter; Json: PUtf8Char;
  ArrayName, EndOfObject: PUtf8Char; opt: TJsonToXmlOptions;
  Pending: boolean): PUtf8Char;
const
    ...
    begin
      ...
      begin
        ...
        repeat
          ...
          Json := AddJsonToXmlNode(W, Json, nil, @info.EndOfObject, opt, true);
          W.AddDirect('<', '/');
          if ArrayName = nil then
            W.AddU(n)
          else
      ...
      begin
        repeat
          ...
          while (Json^ <= ' ') and
            ...
            inc(Json);
          if (Name^ = '@') and
             not (jxoNoAttribute in opt) then
          begin
            if Pending and
               (Name[1] <> #0) and
               not (Json^ in ['{', '[']) then
            begin // '@name':value -> name="value" within the pending start tag
              ...
              Json := AddJsonToXmlNode(W, Json, nil, @info.EndOfObject, opt, false);
              W.AddDirect('"');
            end
          ...
          begin
            ...
            if Json^ = '[' then // arrays are written as list of items, without root
              Json := AddJsonToXmlNode(W, Json, Name, @info.EndOfObject, opt, false)
            else
            begin
              W.Add('<');
              AddXmlEscape(W, Name);
              // no '>' here: the value may start with some '@name' attributes
              Json := AddJsonToXmlNode(W, Json, Name, @info.EndOfObject, opt, true);
              W.AddDirect('<', '/');
              AddXmlEscape(W, Name);
              W.AddDirect('>');
            end;
...
begin // no pending start tag at this level
  result := AddJsonToXmlNode(W, Json, ArrayName, EndOfObject, Options, false);
end;
